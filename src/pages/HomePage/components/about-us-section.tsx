...
const ProductCard = ({ imageSrc, title }: ProductCardProps) => (
    <div className="rounded-2xl overflow-hidden shadow-lg bg-white">
        <img src={imageSrc} alt={title} className="w-full h-64 object-cover" />
        <p className="p-5 font-bold text-gray-800 text-lg">{title}</p>
    </div>
);

const BenefitItem = ({ number, title, description, isOpen, onClick }: BenefitItemProps) => (
    <div className="border-b border-gray-300">
        <button onClick={onClick} className="w-full text-left py-8">
            <div className="flex justify-between items-start gap-6">
                ...
                <ArrowRightIcon
                    className={`w-6 h-6 text-gray-900 flex-shrink-0 mt-2 transition-transform duration-300 ${
        ...
        >
            <div className="min-h-0">
                <div className="pb-8 pl-12 md:pl-16">
                    {description && (
                        <p className="text-gray-600 text-base max-w-lg">{description}</p>
                    )}
...
export const AboutUsSection = () => {
    const [openBenefit, setOpenBenefit] = useState<string | null>('01');

    const handleBenefitClick = (number: string) => {
        setOpenBenefit((prev) => (prev === number ? null : number));
    };
            ...
            <Container>
                <div>
                    ...
                    <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <ProductCard imageSrc="/ai-crop.jpg" title="AI Crop Monitoring" />
                        <ProductCard imageSrc="/product-drone.jpg" title="Drone & Satellite Farming" />
                        <ProductCard imageSrc="/product-analytics.jpg" title="Data-Driven Farm Analytics" />
                    </div>
                </div>
